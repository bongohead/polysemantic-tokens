...
3. Are the sequences adequately dirty, mixing in unique/messy data? For example of things that add diversity:
    ...
    - PDF/OCR extraction and encoding artifacts. Repeated headers/footers mid-paragraph, unhyphenated line breaks, page numbers inline, two-column merge errors, OCR misreads, mojibake, unescaped HTML entities, etc.
    ...
    - Tabular and structured data as text. Misaligned CSV rows, broken Markdown tables, spreadsheet exports with merged-cell artifacts, flattened headers, nutritional info blocks, box scores, etc.
...
13. Again, is every usage of the token correct with the same unambiguous meaning? The unambiguity is very important. Read each sample and confirm the target meaning is clear.